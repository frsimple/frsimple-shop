...
Component({
  ...
  methods: {
    ...
    toChooseItem(e) {
      ...
      // if (!isStock) return;
      const { id, specid, val } = e.currentTarget.dataset;
      ...
      //const { specList, specImgs } = this.properties;
      // let nSpecList = specList.map((row) => {
      //   if (row.id == specId) {
    ...
    handleBuyNumPlus() {
      ...
      if (!isStock || buyNum >= limitMaxCount) return;
      const nextBuyNum = Number(buyNum) + 1;
      this.setBuyNum(nextBuyNum > limitMaxCount ? buyNum : nextBuyNum);
    },
    ...
    handleBuyNumMinus() {
      ...
      const { isStock } = this.properties;
      if (!isStock || buyNum <= limitMinCount) return;
      const nextBuyNum = Number(buyNum) - 1;
    ...
    handleBuyNumChange(e) {
      ...
      this.setBuyNum(nextData.buyNum);
    },
  },
});
