...
const initDicts = async () => {
  let res = await dicVals('SP_AUTHTYPE');
...
const onSubmit = async () => {
  ...
  if (typeof result !== 'object' && result) {
    ...
    if (opt.value === 'add') {
      ...
      try {
        let result1 = await addClient(submitForm);
        if (result1.code === 0) {
      ...
      submitForm.clientId = clientForm.clientId;
      try {
        let result1 = await editClient(submitForm);
        if (result1.code === 0) {
          visibleModal.value = false;
          fetchData();
          MessagePlugin.success('保存成功');
