        ...
        >
          ...
          <template #operation="{ row }">
            <t-space direction="vertical">
              <t-popconfirm theme="danger" content="是否确认收货?" @confirm="orderConfirm(row)">
                <t-button size="small" theme="primary">确认收货</t-button>
              </t-popconfirm>
              <t-button size="small" theme="primary" @click="showDia(row)" variant="outline">修改运单号</t-button>
...
import { queryOrderList, editOrderExpress, confirmOrder } from '@/api/shop/order/list';
...
const showDia = async (order) => {
  ...
  jeForm.value.expressNo = order.express.expressNo;
  jeForm.value.company = order.express.company;
  showJeDialog.value = true;
};
const orderConfirm = async (order) => {
  let msg = MessagePlugin.loading('正在确认收货中...');
  let res = await confirmOrder(order.id);
  MessagePlugin.close(msg);
  if (res.code === 0) {
    MessagePlugin.success('确认收货成功');
    fetchData();
  } else {
    MessagePlugin.error('确认收货失败:' + res.msg);
  }
};
const onConfirmAnother = async () => {
  ...
  if (typeof result !== 'object' && result) {
    ...
    try {
      let res = await editOrderExpress({
        id: jeForm.value.id,
        express: {
      ...
      if (res.code == 0) {
        ...
        MessagePlugin.error('修改失败:' + res.msg);
      }
    } catch (er) {
      MessagePlugin.error('修改失败:' + er);
    } finally {
      confirmBtn.value = {
        content: '确定',
        theme: 'primary',
