        ...
        >
          ...
          <template #goods="{ row }">
            <t-row v-for="(item, index) in row.goods" :key="index" :gutter="10" class="_goodRow">
              <t-col :span="3">
                <t-image :src="item.primaryImage"></t-image>
              </t-col>
              <t-col :span="9">
                <t-row>
                  <t-col :span="12">
          ...
          <template #operation="{ row }">
            <t-button size="small" theme="default" @click="viewOrder(row)">订单详情</t-button>
          </template>
        </t-table>
      </t-card>
    </div>
    <ViewCompont ref="viewDoc"></ViewCompont>
  </div>
</template>
...
import { queryOrderList } from '@/api/shop/order/list';
import { dicVals } from '@/api/common';
import { MessagePlugin } from 'tdesign-vue-next';
import ViewCompont from '../view.vue';
const emit = defineEmits(['reloadCount']);
const viewDoc = ref(null);
const viewOrder = (row) => {
  viewDoc.value.initData(row);
};
//日期选择器
