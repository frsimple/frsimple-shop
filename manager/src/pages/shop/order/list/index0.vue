      ...
      <t-card :bordered="false">
        ...
        >
          ...
          <template #operation="{ row }">
            <t-button size="small" theme="default" :style="{ marginBottom: '5px' }">订单详情</t-button>
            <t-button size="small" theme="primary" @click="showDia(row)">修改订单金额</t-button>
          </template>
        </t-table>
      </t-card>
    </div>
    ...
    >
      <template #body>
        <t-form ref="form" :data="jeForm" :rules="rules">
          <t-form-item label="订单号" name="id" initial-data="TDesign">
            <t-input v-model="jeForm.id" :disabled="true" />
          </t-form-item>
          <t-form-item label="订单金额" name="price" initial-data="123456">
            <t-input-adornment append="元">
              <t-input v-model="jeForm.price" :disabled="true" :style="{ width: '180px' }" />
            </t-input-adornment>
          </t-form-item>
          <t-form-item label="修改订单金额" name="nprice" initial-data="123456">
            <t-input-adornment append="元">
              <t-input-number
                placeholder="请输入要修改的订单金额"
                v-model="jeForm.nprice"
                theme="normal"
                :decimal-places="2"
                :style="{ width: '180px' }"
              ></t-input-number>
...
import { queryOrderList, editOrderPrice } from '@/api/shop/order/list';
...
const rules = {
  nprice: [{ required: true, message: '请输入修改金额', type: 'error' }],
};
...
const showDia = async (order) => {
  ...
  jeForm.value.price = order.nprice ? order.nprice : order.price;
  jeForm.value.nprice = '';
...
const onConfirmAnother = async () => {
  ...
  if (typeof result !== 'object' && result) {
    ...
    try {
      let res = await editOrderPrice({
        id: jeForm.value.id,
        nprice: jeForm.value.nprice,
      });
      if (res.code == 0) {
